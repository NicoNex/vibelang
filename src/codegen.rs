...
fn builtin(name: &str) -> Option<(usize, String)> {
    ...
    match name {
        ...
        "take" => t(2, "vb_take($0, $1)"),
        "drop" => t(2, "vb_drop($0, $1)"),
        "split" => t(2, "vb_split($0, $1)"),
        "lines" => t(1, "vb_lines($0)"),
        "dup" => t(1, "vb_dup($0)"),
        "dict" => t(0, "vb_dict()"),
        "insert" => t(3, "vb_insert($0, $1, $2)"),
        "lookup" => t(2, "vb_lookup($0, $1)"),
        ...
        "parse_i64" => t(1, "vb_parse_int($0, 1, 64)"),
        "parse_u32" => t(1, "vb_parse_int($0, 0, 32)"),
        "parse_u64" => t(1, "vb_parse_int($0, 0, 64)"),
        "parse_f64" => t(1, "vb_parse_f64($0)"),
        "band" => t(2, "vb_band($0, $1)"),
        "bor" => t(2, "vb_bor($0, $1)"),
        "bxor" => t(2, "vb_bxor($0, $1)"),
        "bnot" => t(1, "vb_bnot($0)"),
        "shl" => t(2, "vb_shl($0, $1)"),
        "shr" => t(2, "vb_shr($0, $1)"),
        "ord" => t(1, "vb_ord($0)"),
...
impl<'a> Gen<'a> {
    ...
    fn ex(&mut self, e: &Expr, out: &mut String) -> String {
        match &e.kind {
            ...
            ExprKind::Field(base, f) => {
                ...
                let owner = self.ck.field_owner_at(e.span, f).cloned();
    ...
    fn record(
        ...
        let owner = match self
            .ck
            .field_owner_at(span, fields.first().map_or("", |(f, _)| f))
            .cloned()
        {
            Some(o) => o,
            None => {
    ...
    // (spec §4.6) would put the non-escaping ones on the stack, saving a
    // malloc/free pair per closure.
