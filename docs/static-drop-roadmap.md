...
4. **What `dup` means on an aggregate.** ~~Undecided.~~ **Answered: deep copy.** `dup` is `&a -> a` and `vb_dup` recurses, so the copy owns everything it points at and the two values can be dropped independently. Shared immutable value was never available — §11.3 forbids a refcount — and refusing to copy would have left `own.use_after_move` with no mechanical `fix` on an aggregate, which objective 1 counts as a retry. `VB_CSTR` and `VB_PTR` stay shallow, so a copy holding one still aliases and deep drop must keep suppressing it.
5. **Whether drops can be elided into reuse.** A value dropped immediately before an allocation of the same shape could feed that allocation instead. This is a real optimisation and a real source of bugs; it is not part of this plan and should not be smuggled into Task 8.
   ...
   **Now also the copy.** The `&Vec` operations duplicate every element so that a deep drop cannot free one twice, and that copy is dead work whenever the source dies at the call — which the drop table already knows, because last use is what Task 1 computes. `examples/churn.vibe` prices the unconditional version at +40% wall time (0.05s to 0.07s, repeatable) on a vector of scalars, where `vb_dup` is a call and a switch and no allocation at all. Measure before eliding; the same table that would drive it is the one that would be wrong.
