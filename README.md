...
- **Affine use of owned values.** *Affine* means a value may be used once and not twice. `&` parameters are borrows — lent for the duration of the call and still yours afterwards; everything else is owned and consumed by its first use. Using it twice is an error whose `fix` is `&x`, or `dup x` when the value is a string — `dup` is typed `&Str -> Str`, so offering it on anything else would be a `fix` that does not compile. A `{r with ...}` update on a uniquely owned record mutates in place instead of copying.
