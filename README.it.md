...
- **Uso affine dei valori posseduti.** *Affine* vuol dire che un valore può essere usato una volta e non due. I parametri `&` sono prestiti — dati in prestito per la durata della chiamata e ancora tuoi dopo; tutto il resto è posseduto e viene consumato dal primo uso. Usarlo due volte è un errore il cui `fix` è `&x`, oppure `dup x` quando il valore è una stringa — `dup` ha tipo `&Str -> Str`, quindi offrirlo su altro sarebbe un `fix` che non compila. Un aggiornamento `{r with ...}` su un record posseduto in modo unico muta sul posto invece di copiare.
